...
final class P_DeviceManager
{
    ...
    void disconnectAllForTurnOff(PE_TaskPriority priority)
    {
        for (int i = m_list.size() - 1; i >= 0; i--)
        {
            ...
            {
                device.disconnectWithReason(priority, Status.BLE_TURNING_OFF, DeviceConnectionFailListener.Timing.NOT_APPLICABLE, BleStatuses.GATT_STATUS_NOT_APPLICABLE, BleStatuses.BOND_FAIL_REASON_NOT_APPLICABLE, device.NULL_READWRITE_EVENT());
            }
        }
    }
